...
export function Settings({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(26,24,18,0.3)] backdrop-blur-sm" onClick={onClose}>
      <div
        className="glass-card rounded-[var(--oq-r-lg)] w-full max-w-2xl shadow-card-lg animate-slideUp mx-4 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--border)] bg-[var(--oq-ink-05)]">
          <div className="flex items-center gap-3">
            <div className="oq-logo w-8 h-8 rounded-lg flex items-center justify-center">
            ...
            <div>
              <h3 className="font-display text-xl font-semibold text-[var(--text)]">设置</h3>
              <p className="text-xs text-[var(--muted)]">API、行情源与策略接口</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-5 p-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-[var(--text)]">Finnhub API Key</label>
          ...
          {/* Alpha Vantage */}
          <div className="space-y-2">
            <label className="text-sm font-semibold text-[var(--text)]">Alpha Vantage Key <span className="text-xs text-[var(--muted)] font-normal">可选</span></label>
            <p className="text-xs text-[var(--muted)]">
              美股 K 线备用源（约 25 次/天，
          ...
          {/* LLM */}
          <div className="space-y-2">
            <label className="text-sm font-semibold text-[var(--text)]">LLM 因子配置 <span className="text-xs text-[var(--muted)] font-normal">可选</span></label>
            <p className="text-xs text-[var(--muted)]">
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-[var(--border)] bg-[var(--oq-ink-05)]">
          <button
            type="button"
            onClick={() => {
              onSave(key)
